...
class CommentsControllerTest < ActionController::TestCase
  ...
  class CommentsWhenUserSignedIn < CommentsControllerTest
    ...
    test "PATCH #update fails with valid attributes when comment user is not session user" do
      ...
      patch :update,  format: :html,
                      ...
                      comment: { content: new_content }
      @comment.reload
      assert @comment.content == old_content, "Comment content should not be updated when editing user is not comment author."
  ...
  class CommentsWhenNoCurrentUser < CommentsControllerTest
    def setup
      super
      sign_out
    end

    test "POST #create when no user signed in" do
      test_content = "testing content in some comment creation"
      assert_no_difference('Comment.count') do
        post :create, format: :html,
                      topic_id: @topic.id,
      ...
      assert @topic.comments.last.content == @comment.content, "Last comment object for parent topic should match fixture comment"
      assert_match /Sorry! You need to be signed in to do this/, flash[:notice], "Failed comment create should trigger flash notice"
      assert_redirected_to root_path
    end

    test "PATCH #update when no user signed in" do
      old_content = @comment.content
      new_content = "beep boop beep beep bop boop blip beep boop beep boop beep beep beep bop boop blip beep boop"
      patch :update,  format: :html,
                      topic_id: @topic.id,
                      ...
                      comment: { content: new_content }
      @comment.reload
      assert @comment.content = old_content, "Comment content should not be updated when editing user is not signed in."
      assert_match /Sorry! You need to be signed in to do this/, flash[:notice], "Failed comment create should trigger flash notice"
      assert_redirected_to root_path
    end

    test "DELETE #destroy when no user signed in" do
      assert_no_difference('Comment.count') do
        delete :destroy, format: :html, topic_id: @topic.id, id: @comment.id
      end
      assert_match /Sorry! You need to be signed in to do this/, flash[:notice], "Failed comment create should trigger flash notice"
